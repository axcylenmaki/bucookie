...
if (isset($_GET['remove_avatar'])) {
    if (!empty($user['avatar'])) {
        $file = __DIR__ . '/../assets/uploads/avatars/' . $user['avatar'];
    ...
    header('Location: ' . BASE_URL . 'admin/profile.php?tab=info');
    exit;
}
...
<div style="display:flex;gap:0;border-bottom:1px solid var(--border);margin-bottom:24px">
    <a href="?tab=info"
       style="padding:10px 20px;text-decoration:none;font-size:.83rem;font-weight:500;
              color:<?= $active_tab==='info' ? 'var(--accent)' : 'var(--text-muted)' ?>;
              border-bottom:2px solid <?= $active_tab==='info' ? 'var(--accent)' : 'transparent' ?>;
              margin-bottom:-1px">
        <i class="bi bi-person"></i> Informasi
    </a>
    <a href="?tab=password"
       style="padding:10px 20px;text-decoration:none;font-size:.83rem;font-weight:500;
              color:<?= $active_tab==='password' ? 'var(--accent)' : 'var(--text-muted)' ?>;
              border-bottom:2px solid <?= $active_tab==='password' ? 'var(--accent)' : 'transparent' ?>;
              margin-bottom:-1px">
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-field">
            ...
            <div style="display:flex;align-items:center;gap:20px">
                <div id="avatarPreviewWrap"
                     style="width:80px;height:80px;border-radius:50%;overflow:hidden;flex-shrink:0;border:2px solid var(--border);background:var(--bg-base);display:flex;align-items:center;justify-content:center">
                    <?php if ($avatarUrl): ?>
                    <img id="avatarPreview" src="<?= $avatarUrl ?>"
                         style="width:100%;height:100%;object-fit:cover">
                    <?php else: ?>
                    <div style="width:100%;height:100%;background:var(--accent-soft);display:flex;align-items:center;justify-content:center;font-size:1.6rem;font-weight:700;color:var(--accent)">
                ...
                <div style="flex:1">
                    <label for="avatarInput"
                           style="display:inline-flex;align-items:center;gap:7px;padding:8px 16px;background:var(--bg-card);border:1px solid var(--border);border-radius:8px;cursor:pointer;font-size:.8rem;color:var(--text-secondary)">
        ...
        <div class="mb-field">
            ...
            <textarea name="address" class="form-control"
                      placeholder="Alamat lengkap..."><?= htmlspecialchars($_POST['address'] ?? $user['address'] ?? '') ?></textarea>
        </div>
